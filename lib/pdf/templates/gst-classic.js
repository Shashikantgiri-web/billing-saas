...
export async function renderGstClassicTemplate(doc, { invoice, items, business, settings }) {
  ...
  // ---- ESTIMATE badge (compliance label, always shown) ----
  const badgeW = 30;
  const badgeH = 7;
  ...
  doc.text("ESTIMATE", badgeX + badgeW / 2, badgeY + badgeH / 2 + 2, { align: "center" });
  doc.setTextColor(0, 0, 0);
  doc.setDrawColor(0);
  doc.setFont("helvetica", "normal");

  y = Math.max(headerY, y + 24, badgeY + badgeH) + 2;
  ...
  if (invoice.customer_invoice_number) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(8);
    doc.setTextColor(90, 100, 121);
    doc.text(`Ref: ${invoice.customer_invoice_number}`, contentRight, y, { align: "right" });
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(9);
  }
  doc.setFont("helvetica", "bold");
  ...
  // ---- Item table (dynamic Kg/Liter columns) ----
  ...
  const extraCols = (showKg ? 1 : 0) + (showLiter ? 1 : 0);
  const itemColWidth = 62 - extraCols * 14;

  const cols = [
    { key: "sr", label: "Sr", w: 8, align: "left" },
    { key: "item", label: "Item", w: itemColWidth, align: "left" },
    { key: "qty", label: "Qty", w: 14, align: "right" },
    ...(showKg ? [{ key: "kg", label: "Kg", w: 14, align: "right" }] : []),
    ...(showLiter ? [{ key: "liter", label: "Liter", w: 14, align: "right" }] : []),
  ...
  // X coordinate of the right edge of the Qty column, for the totals row.
  let qtyColRightEdge = tableLeft;
  {
    let acc = tableLeft;
    for (const c of cols) {
      acc += c.w;
      if (c.key === "qty") { qtyColRightEdge = acc; break; }
    }
  }
  ...
  items.forEach((item, idx) => {
    ...
    const rowCells = [idx + 1, item.product_name, item.quantity];
    if (showKg) rowCells.push(item.kg_value != null ? Number(item.kg_value).toFixed(3) : "—");
    if (showLiter) rowCells.push(item.liter_value != null ? Number(item.liter_value).toFixed(3) : "—");
    rowCells.push(
      "Nos",
    ...
    drawRow(rowCells, y, rowH);
    y += rowH;
  });

  // ---- Total Qty / Total Amount summary row ----
  ...
  // Redraw the column grid over the fill so the total row stays visually
  // part of the same table instead of floating as a separate block.
  doc.setDrawColor(0);
  doc.setLineWidth(0.2);
  doc.rect(tableLeft, y, tableWidth, rowH);
  let totalColX = tableLeft;
  cols.forEach((c) => {
    doc.line(totalColX, y, totalColX, y + rowH);
    totalColX += c.w;
  });
  doc.line(totalColX, y, totalColX, y + rowH);
  ...
  doc.text("Total", tableLeft + 2, y + rowH - 2.5);
  doc.text(String(totalQty), qtyColRightEdge - 2, y + rowH - 2.5, { align: "right" });
  doc.text(totalAmount.toFixed(2), tableLeft + tableWidth - 3, y + rowH - 2.5, { align: "right" });
  doc.setFont("helvetica", "normal");
  y += rowH;
  ...
  // Derived from this invoice's actual tax, not the Settings default — the
  // default can drift out of sync with what a given invoice actually charged.
  const effectiveGstPercent = subtotal > 0 ? Math.round((taxTotal / subtotal) * 100 * 100) / 100 : 0;
  const cgstPct = Number((effectiveGstPercent / 2).toFixed(2));
  const sgstPct = Number((effectiveGstPercent / 2).toFixed(2));
  ...
  totalLine(`CGST (${cgstPct}%)`, cgst);
  totalLine(`SGST (${sgstPct}%)`, sgst);
  ...
  terms.forEach((term) => {
    ...
    const wrappedTerm = doc.splitTextToSize(line, contentRight - marginX - 60);
    doc.text(wrappedTerm, marginX + 2, y);
    y += wrappedTerm.length * 4;
  });
