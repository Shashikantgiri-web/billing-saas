...
export async function renderGstClassicTemplate(doc, { invoice, items, business, settings }) {
  ...
  doc.setDrawColor(180, 188, 210);
  doc.setLineWidth(0.4);
  doc.line(tableLeft, y, tableLeft + tableWidth, y);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  doc.setTextColor(26, 26, 26);
  doc.text("Total", tableLeft + 10, y + rowH / 2 + 2.5);
  doc.text(String(totalQty), qtyColRightEdge - 2, y + rowH / 2 + 2.5, { align: "right" });
  doc.text(totalAmount.toFixed(2), tableLeft + tableWidth - 3, y + rowH / 2 + 2.5, { align: "right" });
  ...
  const totalGstPercent = Number(settings?.default_tax_percent || 0);
  const cgstPct = totalGstPercent / 2;
  const sgstPct = totalGstPercent / 2;
