...
/**
 * Loads an image URL into a base64 data URL for jsPDF's addImage.
...
export async function renderGstClassicTemplate(doc, { invoice, items, business, settings }) {
  ...
  // ---- Header ----
  let textX = marginX + 3;
  const logoDataUrl = await loadImageAsDataUrl(settings?.logo_url);
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, "PNG", marginX + 2, y, 20, 20);
      textX = marginX + 26;
    } catch {
      // Unsupported format (e.g. SVG) — skip the logo, keep text layout.
    }
  }
  ...
  doc.setFontSize(8.5);
  let headerY = y + 11;
  if (business?.location) { doc.text(business.location, textX, headerY); headerY += 4; }
  ...
  y += 4.5;
  doc.setFontSize(8.5);
  if (customer.address) { doc.text(customer.address, marginX + 2, y); y += 4; }
  ...
  const sigDataUrl = await loadImageAsDataUrl(settings?.signature_url);
  const sigY = y - terms.length * 4;
  if (sigDataUrl) {
    try {
      doc.addImage(sigDataUrl, "PNG", contentRight - 40, sigY - 12, 30, 14);
    } catch {
      // Unsupported image format — fall back to a blank signature line only.
    }
  }
  doc.line(contentRight - 45, sigY + 4, contentRight - 2, sigY + 4);
  doc.setFontSize(8);
  doc.text(`For ${business?.name || "the Business"}`, contentRight - 2, sigY + 8, { align: "right" });
  doc.text("Authorized Signatory", contentRight - 2, sigY + 12, { align: "right" });
